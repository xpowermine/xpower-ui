    ...
    $sender(
        ...
        return <NftTxSender
            ...
            onSent={() => {
                ...
                /**
                 * @todo: ugly reset; move somehow into NftTxTarget!
                 */
                const id = Nft.coreId({
                    issue: nft_issue, level: nft_level
                });
                const $el: React.RefObject<HTMLInputElement>
                    = this.global_ref<HTMLInputElement>(`nft-target:${id}`);
                if ($el.current) {
                    $el.current.value = $el.current.defaultValue;
                }
            }}
        />;
    }
